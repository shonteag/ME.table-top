...
class Enemy(object):
	...
	def enemy_cast_on_self(self, power_key):
	...
	def take_damage(self, amount, source=None):
		if amount <= self.properties['shield']:
			...
			self.properties.update({'shield':int(newshield)})
			print str(self.properties['name']) + " (enemy) takes " + str(amount) + " damage to shields (" + str(newshield) + " remaining) (" + str(source) + ")."
			...
			if remainder - int((math.ceil(remainder * self.properties['damage_reduction']))) <= self.properties['armor']:
				...
				print str(self.properties['name']) + " (enemy) takes " + str(armor_damage) + " damage to armor. (" + str(newarmor) + ") (" + str(source) + ")."
				...
				print str(self.properties['name']) + " (enemy) takes " + str(shielddamage) + " to shield (0 remaining) (" + str(source) + ")."
				print str(self.properties['name']) + " (enemy) takes " + str(armor_damage) + " to armor (0 remaining) (" + str(source) + ")."
				print str(self.properties['name']) + " (enemy) takes " + str(remainder) + " to health (" + str(newhealth) + " remaining) (" + str(source) + ")."

				if self.properties['health'] <= 0:
					#target dead
					print str(self.properties['name']) + " (enemy) is dead."
					self.is_alive = False
	# end take_damage
	...
	def print_combat_info(self):
		print "---- Enemy: " + str(self.properties['name']) + ", Level " + str(self.properties['level']) + " " + str(self.properties['class_key']) + ", Difficulty Multiplier " + str(self.properties['difficulty'])
		print "Negative Effects: "
		for key,turns in self.get_negative_effects().iteritems():
			print " " + str(key) + " : " + str(turns)

		print "\nCooldowns: "
		for key,turns in self.get_cooldowns().iteritems():
			print " " + str(key) + " : " + str(turns)

		print "\nPrimes: "
		print " Tech primed turns: " + str(self.get_tech_primed())
		print " Biotic primed turns: " + str(self.get_biotic_primed())

		print "\nHealth and shields:"
		print " " + str(self.get_health_shields()[0]) + "hp, " + str(self.get_health_shields()[1]) + "sp, (" + str(self.properties['shield_recharge']) + " sr/turn)"
	# end print_combat_info
...
if __name__ == "__main__":
	...
	shield = int(raw_input("shield: "))
	shield_recharge = int(raw_input("  recharge: "))
	armor = int(raw_input("armor: "))
	damage_reduction = float(raw_input("  damage_reduction: "))
	powers = str(raw_input("powers (separated by space): "))
	powers = powers.split()

	powerh = PowerHandler()
	classh = ClassHandler()

	Enemy = Enemy(name='Test Name',
				  class_key='SOL',
				  race='other',
				  role='none',
				  level=1,
				  ...
				  powers=powers,
				  description='None',
				  power_handler=powerh,
				  class_handler=classh,
				  stats=None)

	print "Command Cycling: "
	while True:
		command = raw_input(" >")
		...
		if command[0] == 'exit':
			...
			Enemy.new_turn()
			print "Negative Effects: "
			for key,turns in Enemy.get_negative_effects().iteritems():
				print " " + str(key) + " : " + str(turns)

			print "\nCooldowns: "
			for key,turns in Enemy.get_cooldowns().iteritems():
				print " " + str(key) + " : " + str(turns)

			print "\nPrimes: "
			print " Tech primed turns: " + str(Enemy.get_tech_primed())
			print " Biotic primed turns: " + str(Enemy.get_biotic_primed())

			print "\nHealth, armor, and shields:"
			print " " + str(Enemy.get_health_shields()[0]) + "hp, " + str(Enemy.properties['armor']) + "armor, " + str(Enemy.get_health_shields()[1]) + "sp"

		elif command[0] == 'take':
			if command[1] == 'power':
				Enemy.enemy_cast_on_self(command[2])
			elif command[1] == 'damage':
				Enemy.take_damage(int(command[2]))

		elif command[0] == 'use':
			if command[1] == 'power':
				Enemy.use_power(command[2])